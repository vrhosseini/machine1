...
class simplePINN(nn.Module):
    def __init__(self):
        ...
        self.sequential=nn.Sequential(
            ...
            nn.Linear(in_features=40, out_features=5,bias=True)
...
def LossPDE(Collo_x,Collo_y):
    ...
    return loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y


def LossPDE1(Collo_x,Collo_y):
    ...
    loss_sxx=loss_f(s11,sxx_exact)
    loss_syy=loss_f(s22 ,syy_exact)
    loss_sxy=loss_f(s12,sxy_exact)

    
    return loss_u,loss_v,loss_sxx,loss_syy,loss_sxy
...
def Loss_Neum_BC(x_Neum,y_Neum,tx_Neum,ty_Neum, nx_Neum, ny_Neum):
    ...
    # nx_Neum =torch.zeros_like(x_Neum).to(device)
    # ny_Neum =(torch.zeros_like( y_Neum)).to(device)    
...
num_dom_train_samples =50
# number of boundary training samples
num_b_train_samples = 50
...
def Boundary_Condition2(num_b_train_samples): #top   
    ...
    tx_top=mu*np.pi*(-np.cos(2*np.pi*x_top)+np.cos(np.pi*x_top)*Q/4)
...
x_total=np.hstack((Collo_x,x_left,x_top,x_right,x_bottom))
y_total=np.hstack((Collo_y,y_left,y_top,y_right,y_bottom))
print(x_total.shape) 
print(y_total.shape) 
batchsize=5000
...
# loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y,,loss_v,loss_sxx,loss_syy,loss_sxy  = LossPDE(Collo_x, Collo_y)
# loss_u= loss_u.cpu().detach().numpy()
# loss_syy = loss_syy.cpu().detach().numpy()
# loss_sxy=loss_sxy.cpu().detach().numpy()
...
# dataset = TensorDataset(x_total,x_total)
...
optimizer1 = optim.AdamW(modelv1.parameters(), lr=learning_rate, betas = (0.9,0.99),eps = 10**-9)
#optimizer1 = torch.optim.Adam(modelv1.parameters(), lr=learning_rate,betas=(0.9, 0.999), eps=1e-20, weight_decay=0.001, amsgrad=False)
    ...
    for epoch in range(epochs):
      #for batch_idx, (x, t) in enumerate(dataloader):
            def closure():
                ...
                loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y = LossPDE(x_total, y_total)
                
                loss_gov = loss_f_s11r+ loss_f_s22r+ loss_f_s12r+ lossf_x+lossf_y
                
                loss_u,loss_v,loss_sxx,loss_syy,loss_sxy  = LossPDE1(x_total, y_total)
                loss_gov1 = loss_u+loss_v+loss_sxx+loss_syy+loss_sxy                
        ...
        # Print loss information every 100 epochs
            if epoch % 100 == 0:
                ...
                print('Train Epoch: {} \tLoss: {:.10f} \tLoss_u: {:.6f}\tlossf_x: {:.6f}\tloss_tx: {:.6f}\tloss_ty: {:.6f}'.format(epoch, loss.item(),lossf_x.item(),lossf_y.item(),loss_tx.item(),loss_ty.item()))
                #print('Train Epoch: {} \tLoss: {:.10f} \tLoss_u: {:.6f}\tlossf_x: {:.6f}\tloss_tx: {:.6f}\tloss_ty: {:.6f}'.format(epoch, loss.item(),lossf_x.item(),lossf_y.item(),loss_tx.item(),loss_ty.item()))
