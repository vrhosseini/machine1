...
class simplePINN(nn.Module):
    def __init__(self):
        ...
        self.sequential=nn.Sequential(
            ...
            nn.Linear(in_features=20, out_features=20,bias=True),
            Swish(),
            nn.Linear(in_features=20, out_features=5,bias=True)
...
def LossPDE(Collo_x,Collo_y, n1=0, n2=0):
    ...
    sample1 = torch.cat((Collo_x, Collo_y),  dim=1)
...
def Loss_Neum_BC(x_bottom,y_bottom,tx_bottom,ty_bottom):
     x_bottom =torch.Tensor(x_bottom[:,None] ).to(device)
     y_bottom =torch.Tensor(y_bottom [:,None]).to(device)
...
num_dom_train_samples = 5
# number of boundary training samples
num_b_train_samples = 10  
...
def Boundary_Condition1(num_b_train_samples):    
...
def Boundary_Condition2(num_b_train_samples):    
...
def Boundary_Condition3(num_b_train_samples):    
    #Boundary Condition x=0 and 0<y<1
...
def Boundary_Condition4(num_b_train_samples):    
    #Boundary Condition 0<x<1 and y=0
...
x_Neum=np.hstack((x_top,x_right,x_left))
y_Neum=np.hstack((y_top,y_right,y_left))

tx_Neum=np.hstack((tx_top,tx_right,tx_left))
ty_Neum=np.hstack((ty_top,ty_right,ty_left))
...
optimizer1 = optim.Adam(modelv1.parameters(), lr=learning_rate, betas = (0.9,0.99),eps = 10**-9)
# optimizer = torch.optim.Adam(modelv2.parameters(), lr=0.01,betas=(0.9, 0.999), eps=1e-09, weight_decay=0.001, amsgrad=False)
    ...
    for epoch in range(epochs):
      #for batch_idx, (x, t) in enumerate(dataloader):
            def closure():
                ...
                loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y  = LossPDE(Collo_x,Collo_y, n1=0, n2=0)
                ...
                losss_Dir=loss_Dir_u+loss_Dir_v
                loss=loss_gov+losss_Dir
                loss.backward()
        ...
        # Print loss information every 100 epochs
            if epoch % 100 == 0:
                ...
                loss = loss_f_s11r+ loss_f_s22r+ loss_f_s12r+lossf_x+lossf_y
