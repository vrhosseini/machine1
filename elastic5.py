...
def LossPDE(Collo_x,Collo_y, n1=0, n2=0):
    ...
    u,v,_,_,_=analytical_soln(lamd, mu, Q, Collo_x,Collo_y)
    ...
    u_exact=torch.Tensor(u[:,None] ).to(device)
    v_exact=torch.Tensor(v[:,None] ).to(device)
    ...
    loss_v=loss_f(v_exact,u)
    
    return loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y,loss_u,loss_v
...
num_dom_train_samples = 20
# number of boundary training samples
num_b_train_samples = 80
    ...
    for epoch in range(epochs):
      #for batch_idx, (x, t) in enumerate(dataloader):
            def closure():
                ...
                loss_f_s11r, loss_f_s22r, loss_f_s12r,lossf_x,lossf_y,loss_u,loss_v  = LossPDE(Collo_x,Collo_y, nx_Neum, ny_Neum)
                
                loss_gov = loss_f_s11r+ loss_f_s22r+ loss_f_s12r+lossf_x+lossf_y+loss_u+loss_v 
